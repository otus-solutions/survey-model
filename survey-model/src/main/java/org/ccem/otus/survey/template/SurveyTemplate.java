...
public class SurveyTemplate {
  ...
  public List<String> getGridIntegerCustomIds(){
    return itemContainer.stream()
      .filter(item -> item instanceof GridIntegerQuestion)
      .flatMap(item -> ((GridIntegerQuestion) item).lines.stream())
      .flatMap(line -> line.gridIntegerList.stream()
    ...
    return itemContainer.stream()
      ...
      .flatMap(line -> line.gridTextList.stream()
        .map(gridTextItem -> gridTextItem.customID))
      .collect(Collectors.toList());
  }
